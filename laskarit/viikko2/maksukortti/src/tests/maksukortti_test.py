...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_negatiivisen_summan_lataaminen_ei_aiheuta_ongelmia(self):
        self.kortti.lataa_rahaa(-10)

        self.assertEqual(str(self.kortti), "Kortilla on rahaa 10.00 euroa")
    
    def test_voi_ostaa_edullisen_lounaan_vaikka_tili_tyhjenee(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(str(kortti), "Kortilla on rahaa 0.00 euroa")

    def test_voi_ostaa_maukkaan_lounaan_vaikka_tili_tyhjenee(self):
